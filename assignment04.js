...
var status = "undefined"; // variable for the status
...
function sendValues () {
    if(actualValue > desiredValue - 10 && actualValue < desiredValue +10) {
        status = "OK";
        console.log("OK");
    }
    if(actualValue <= desiredValue - 10) {
        status="low";
        console.log("low");
    }
    if(actualValue >= desiredValue + 10) {
    ...
    wss.broadcast(JSON.stringify({"type": "clientReadValues", "desiredValue": desiredValue, "actualValue": actualValue, "status": status}));
};